...
def show_quetion():
        ...
        RadioGroup.setExlusive(False)
        rbtn_1 setChecked(False)
        rbtn_2 setChecked(False) 
        rbtn_3 setChecked(False)
        rbtn_4 setChecked(False)
        RadioGroup.setExlusive(True)
...
def test():
        ...
        1b_Quetion.setText(q.quetion)
        1b_Correct.setTextq.right_answer)
        show_quetion()


def show_correct(res):
        1b_Result.setText(res)
        show_result()

def check_answer():
        ''' если выбран какой-то выриант ответа, то надо проверить и показать панель правельных ответов'''
        if answers[0].isChecked():
                show_correct('Правельно!')
                window.score += 1
                print('статистика\n-Всего вопрсов ', window.total, '\n-Правельных ответов: ', window.score)
                print('Рейтинг: ', (window.score/window.totsl*100), '%')
